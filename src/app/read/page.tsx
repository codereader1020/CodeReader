...
export default function ReadPage() {
  ...
  const processImageFile = async (file: File) => {
    ...
    try {
      const results = await decodePdf417(file);
      if (results && results.length > 0) {
        ...
        saveHistoryItem({
          ...
          title: `Decoded PDF417 (${file.name})`,
          subtitle: `${results[0].text.length} characters`,
        });
      }
    } catch (err: unknown) {
      const error = err as Error;
      setErrorMsg(error?.message || 'No readable PDF417 barcode found in the provided image.');
    } finally {
      setIsDecoding(false);
    }
  };
      ...
      decodePdf417(canvas)
        .then((results) => {
          if (results && results.length > 0) {
            ...
            stopCamera(); // Stop camera on successful scan match
  ...
  // Helper to format decoded JSON safely
    ...
    <div className="space-y-8 pb-12">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          Decode PDF417 barcodes from uploaded images or live video camera streams. Validates structured employee credentials and digital signatures locally.
        </p>
      </div>
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          {activeMode === 'upload' && (
            <div
              ...
              onDrop={handleDrop}
              className={`glass-panel p-8 rounded-2xl border-2 border-dashed text-center transition-all flex flex-col items-center justify-center min-h-[340px] relative ${
                dragActive
              ...
              {imagePreviewUrl ? (
                <div className="space-y-4">
                  ...
                  <img
                    ...
                    className="max-h-48 rounded-xl object-contain mx-auto border border-gray-800 shadow-lg"
                  />
                  <p className="text-xs text-gray-400">
                    {selectedFile?.name} ({(selectedFile?.size || 0) / 1024 < 1024 ? `${((selectedFile?.size || 0) / 1024).toFixed(1)} KB` : `${((selectedFile?.size || 0) / 1048576).toFixed(1)} MB`})
                  </p>
                  <span className="inline-flex items-center gap-1.5 text-xs text-blue-400 underline cursor-pointer">
                    <RefreshCw className="w-3.5 h-3.5" /> Choose another image
                  </span>
                </div>
              ) : (
                <div className="space-y-3 pointer-events-none">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white">Drag and drop your PDF417 image here</p>
                    <p className="text-xs text-gray-400 mt-1">Supports PNG, JPG, WEBP, BMP</p>
                  </div>
                ...
                <div className="relative rounded-2xl overflow-hidden bg-black aspect-video flex items-center justify-center border border-gray-800">
                  ...
                  {isCameraActive && (
                    <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center">
                      <div className="w-64 h-32 border-2 border-blue-500 rounded-xl relative shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                        <div className="absolute inset-x-0 h-0.5 bg-blue-400 shadow-[0_0_10px_#60a5fa] animate-pulse top-1/2"></div>
                      </div>
                      <span className="text-xs font-semibold text-white mt-4 bg-black/60 px-3 py-1 rounded-full backdrop-blur-md">
                        Align PDF417 code within box
                      </span>
                    </div>
                  )}
                </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-gray-800 space-y-6 min-h-[420px]">
            ...
            {isDecoding ? (
              ...
              <div className="p-6 rounded-xl bg-red-500/10 border border-red-500/20 text-center space-y-2">
                ...
                <h4 className="text-sm font-semibold text-red-300">Decoder Error</h4>
